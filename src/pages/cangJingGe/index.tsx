  ...
  return (
    <View className='cang'>
      ...
      <View className='cang-select-container'>
        <View className='cang-select-item'></View>
      </View>
      <View className='cang-wrapper'>
        <View className='cang-left'>
          {
            zhuangBeiTypeList.map(z => <View className={`cang-type ${zId === z.id ? 'active' : ''}`} key={z.id} onClick={() => setZId(z.id)}>{z.name}</View>)
          }
        </View>
        <View className='cang-right'>
          {
            zhuangBeiList.map(z => (
